      ...
      <div style="background:var(--td-bg-surface-elevated); border:1px solid var(--td-border); border-radius:0.75rem; padding:2rem; margin-bottom:2rem;">
        <div style="display:flex; flex-wrap:wrap; justify-content:space-between; align-items:flex-start; gap:1.5rem;">
          <div>
            ...
            <!-- Submit Community Test CTA -->
            <div style="margin-top:1rem;">
              <a href="#/submit?device=${encodeURIComponent(device.id)}" style="display:inline-flex; align-items:center; gap:0.4rem; padding:0.45rem 1rem; border-radius:0.375rem; background:var(--td-info); color:#fff; font-weight:600; font-size:0.85rem; text-decoration:none; white-space:nowrap;">
                <span>🧪</span> <span>Submit Community Test for this Device</span>
              </a>
            </div>
          </div>

          <!-- Quick Stats Badge Box -->
          <div style="background:var(--td-bg-card); border:1px solid var(--td-border-subtle); border-radius:0.5rem; padding:1rem 1.5rem; text-align:center; min-width:160px;">
            <div style="font-size:0.75rem; text-transform:uppercase; letter-spacing:0.05em; color:var(--td-text-muted); margin-bottom:0.25rem;">Experiments Involved</div>
            <div style="font-size:1.75rem; font-weight:700; color:${linked.length > 0 ? 'var(--td-info)' : 'var(--td-text-muted)'};">${linked.length}</div>
            <div style="font-size:0.75rem; color:var(--td-text-secondary); margin-top:0.25rem;">${linked.length > 0 ? 'Empirical Records' : 'Baseline Catalog'}</div>
          </div>
        </div>
      </div>
